...
class ContactSuggestionsTest {
    ...
    fun `a blank query offers nothing, so the dropdown does not open on an empty field`() {
        assertEquals(emptyList<ContactSuggestion>(), ContactSuggestions.forQuery(contacts, recognizer, "", emptySet(), "US"))
    }

    @Test
    fun `matching contacts come back as person suggestions`() {
        val results = ContactSuggestions.forQuery(contacts, recognizer, "smith", emptySet(), "US")
    ...
    fun `a person suggestion carries the display name on its own, not folded into the label`() {
        ...
        val result = ContactSuggestions.forQuery(contacts, recognizer, "john", emptySet(), "US").single() as PersonSuggestion
        assertEquals("John Smith", result.displayName)
    }

    @Test
    fun `an already-selected contact is not suggested again`() {
        val results = ContactSuggestions.forQuery(contacts, recognizer, "smith", setOf("5551112222"), "US")
    ...
    fun `a name match that starts with the query sorts before one that only contains it`() {
        ...
        val results = ContactSuggestions.forQuery(withPrefixMatch, recognizer, "smith", emptySet(), "US")
    ...
    fun `a real phone number leads the list ahead of any name matches`() {
        ...
        val results = ContactSuggestions.forQuery(phoneLikeContacts, recognizer, "5551234567", emptySet(), "US")
    ...
    fun `matching by digits finds the contact without typing their name`() {
        ...
        val results = ContactSuggestions.forQuery(contacts, recognizer, "1112222", emptySet(), "US")
    ...
    fun `a query matching a contact's exact number does not also duplicate it as a new number`() {
        ...
        val results = ContactSuggestions.forQuery(contacts, recognizer, "5551112222", emptySet(), "US")
    ...
    fun `no match and no possible number returns an empty list`() {
        assertEquals(emptyList<ContactSuggestion>(), ContactSuggestions.forQuery(contacts, recognizer, "xyz", emptySet(), "US"))
    }
}
