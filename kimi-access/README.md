...
deliberate tell that this is not an official Moonshot product. Launcher
fallback artwork is code-drawn in `tools/gen_icons.py` (exact 24-unit grid
geometry, no rotate-and-guess); the widgets themselves render committed
vector drawables, tinted at runtime.
...
├── tools/gen_icons.py                 # launcher fallback PNGs, code-drawn
...
apksigner`. Only the launcher fallback PNGs are generated by
`tools/gen_icons.py` at build time and intentionally **not committed** —
the K mark, overflow glyph and picker previews are committed vector
drawables, and the generator prunes stale raster leftovers from older runs
so they can't collide with the vectors at aapt2 time. On slow/flaky
filesystems, redirect intermediates:
...
* **v0.2.3** — themed widgets (user colors, fluid sizing, Acid Wire
  identity with vector drawables); build fix: `tools/gen_icons.py` no
  longer emits the old `drawable/*.png` artwork (`ic_k`, `ic_more`,
  `preview_mic`, `preview_projects`) that collided with the committed
  vectors, and `build.sh` regenerates launcher fallbacks unconditionally.
